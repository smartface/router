...
type NativeStackRouterParams<Ttarget = any> = RouterParams<Ttarget> &
	({ modal?: true; modalType?: "bottom-sheet"; bottomSheetOptions?: BottomSheetOptions } | { modal?: true; modalType?: "modal"; modalOptions?: any } | { modal?: false });
 ...
 * @since 1.0.0
 */
export default class NativeStackRouter<Ttarget = Page> extends NativeRouterBase<Ttarget> {
	/**
	...
	static of<Ttarget = Page>(params: NativeStackRouterParams<Ttarget>) {
		params.renderer = createRenderer(new NavigationController() as any);
	...
	constructor(params: NativeStackRouterParams<Page>) {
		...
		if (params.modal) this._modalType = params.modalType || "modal";
		...
		if (this._renderer?._rootController instanceof NavigationController && typeof this._headerBarParams === "function") {
			(this._renderer._rootController as ControllerType).headerBar = this._headerBarParams();
		}
	}
	...
	setHeaderBarParams(params: Partial<HeaderBar>) {
		if (this._renderer?._rootController?.headerBar) this._renderer._rootController.headerBar = params;
	}
				...
				if (action.operation === NavigationController.OperationType.POP && !this._fromRouter) {
					try {
						...
						if (typeof this.dispatch === "function") {
							this._historyController?.history && this.dispatch(this._historyController.history.location, "POP", this, false);
						}
					} catch (e) {
						throw e;
					}
	...
	pushHomeRouteBefore(path: string) {
		if (this.hasHome() && this._renderer?._rootController && this._renderer?._rootController instanceof NavigationController) {
	...
	onRouteEnter(route: Route, requestedUrl: string) {
		...
		switch (action) {
			case "REPLACE":
				if (this._fromRouter) {
					this._renderer?.replaceChild((route instanceof Router && route.renderer && route.renderer._rootController) || view, 0);
				}
				break;
			case "PUSH":
				if (this._fromRouter) {
					if (route instanceof NativeStackRouter && route.isModal() && !this._presented && !active) {
						try {
							...
							const onDismissComplete = (hooks?: DismissHook) => {
								...
								if (_backUrl && !hooks?.after && !hooks?.before && typeof this.dispatch === "function") {
							...
							const normalizeHistory = (hooks?: DismissHook) => {
								...
								if (hooks?.before && typeof this.dispatch === "function" && this._historyController?.lastLocation) {
									this.dispatch(this._historyController?.lastLocation, "POP", this, false);
									hooks.before();
								}
							};
							...
							this._renderer?.present({
								controller: route instanceof Router && route.renderer ? route.renderer._rootController : view,
					...
					} else if ((!(route instanceof NativeStackRouter) || !route.isModal()) && !active) {
						this._currentRouteUrl = url || undefined;
						try {
							this._renderer?.pushChild(((route as Router).renderer && (route as Router).renderer?._rootController) || view, this.isAnimated());
						} catch (e) {
			...
			case "POP":
				...
				if (this._fromRouter) {
					if ((!(route instanceof NativeStackRouter) || (route instanceof NativeStackRouter && !route._dismiss && !route.isModal())) && exact) {
	...
	getStepLengthFromCurrent(url: string) {
		...
		const index = this._historyController?.findIndex((location) => location.url === url) || 0;
