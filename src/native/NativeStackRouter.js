...
class NativeStackRouter extends Router {
  ...
  static of ({ path = "", build = null, routes = [], exact = false, renderer = null }) {
    return new NativeStackRouter({ path, build, routes, exact, renderer: createRenderer((build && build()) || new NavigationController())});
  }
  ...
  onHistoryChange(location, action){
    const view = this.render(location.state);
    console.log("stack view : "+view+" : "+action);
    if(!view) return;
    
    switch (action) {
      case 'PUSH':
        this._renderer._rootPage.push(view);
        break;
      case 'POP':
        this._renderer._rootPage.pop();
        break;
    }
  }
}
